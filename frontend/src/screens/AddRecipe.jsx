...
import { addRecipe } from "../store/slices/recipe";
...
export default function AddRecipeScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    if (!recipe.title.trim()) {
      setErrorAlert({ visible: true, message: "Please enter a recipe title" });
      return;
    }
    ...
    if (cleanedIngredients.length === 0) {
      setErrorAlert({
        visible: true,
        message: "Add at least one valid ingredient",
      });
      return;
    }
    ...
    try {
      ...
      dispatch(addRecipe(finalRecipeData))
        .unwrap()
        .then(() => setShowAlert(true))
        .catch((err) =>
          setErrorAlert({
            visible: true,
            message: err || "Failed to add recipe",
          })
        )
      ...
      setErrorAlert({
        visible: true,
        message: "Failed to process image or prepare recipe",
      });
      setIsLoading(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={styles.inputSection}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              handleInputChange("isVegetarian", !recipe.isVegetarian)
            }
          >
            <Ionicons
      ...
      <CustomAlert
        ...
        onCancel={() => setErrorAlert({ visible: false, message: "" })}
      />
    </KeyboardAvoidingView>
  );
}

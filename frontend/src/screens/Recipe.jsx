...
import { View, Text, Image, StyleSheet, ScrollView, TouchableOpacity, Dimensions, Share, Alert, Modal, TextInput } from 'react-native';
...
export default function Recipe({navigation}) {
    ...
    const handleAddReview = async () => {
        if (newReview.trim() === '' || newRating === 0) {
            Alert.alert('Incomplete Review', 'Please provide both a rating and review text.');
            return;
        }
        ...
        try {
            ...
            Alert.alert('Review Added', 'Your review has been added successfully!');
        } catch (error) {
    ...
    const averageRating = recipe?.reviews?.length > 0
        ? (recipe.reviews.reduce((sum, review) => sum + review.rating, 0) / recipe.reviews.length).toFixed(1)
        : "4.8";
        ...
        <View style={styles.container}>
            ...
            <View style={styles.imgContainer}>
                ...
                <View style={styles.actionButtons}>
                    <TouchableOpacity style={styles.actionButton} onPress={handleLikeOrUnlike}>
                        <Ionicons
                ...
                <View style={styles.contentContainer}>
                    ...
                    <View style={styles.recipeHeader}>
                        <View style={styles.titleSection}>
                            <Text style={styles.recipeTitle}>{recipe?.title || 'Delicious Recipe'}</Text>
                            <Text style={styles.recipeCategory}>Main Course • Dinner</Text>
                        </View>
                        <View style={styles.ratingContainer}>
                            ...
                            <TouchableOpacity onPress={() => setShowReviewsList(true)}>
                                <Text style={styles.reviewCount}>({recipe?.reviews?.length || 324} reviews)</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                    ...
                    <View style={styles.section}>
                        ...
                        <Text style={styles.descriptionText}>
                            {recipe?.description || "A delicious and easy-to-make recipe that brings together the perfect blend of flavors and textures. This dish is perfect for any occasion and will surely impress your family and friends."}
                        </Text>
                    </View>
                    ...
                    <View style={styles.section}>
                        <View style={styles.sectionHeader}>
                            ...
                            <TouchableOpacity style={styles.servingAdjuster}>
                                <Ionicons name="remove-circle-outline" size={20} color="#4ECDC4" />
                        ...
                        {(recipe?.ingredients || [
                            ...
                            <View key={index} style={styles.ingredientCard}>
                                ...
                                <View style={styles.ingredientInfo}>
                                    <Text style={[styles.ingredientName, checkedIngredients.has(index) && styles.checkedText]}>
                                        {ingredient.name}
                                    </Text>
                                    <Text style={[styles.ingredientAmount, checkedIngredients.has(index) && styles.checkedText]}>
                                ...
                                >
                                    <Ionicons
                                        ...
                                        size={24}
                                        color={checkedIngredients.has(index) ? "#4ECDC4" : "#7F8C8D"}
                                    />
                                </TouchableOpacity>
                            </View>
                        ))}
                    ...
                    <View style={styles.actionSection}>
                        <TouchableOpacity style={styles.primaryButton} onPress={() => navigation.navigate('CookingSteps', { recipe })}>
                            <Ionicons name="play-circle" size={24} color="white" />
                            <Text style={styles.primaryButtonText}>Start Cooking</Text>
                        </TouchableOpacity>

                        <View style={styles.secondaryButtons}>
                            <TouchableOpacity style={styles.secondaryButton} onPress={handleSaveOrUnsave}>
                                <Ionicons
                                    name={isSaved ? "bookmark" : "bookmark-outline"}
                                    size={20}
                                    color={isSaved ? "#4ECDC4" : "black"}
                                />
                                <Text style={[styles.secondaryButtonText, { color: isSaved ? "#4ECDC4" : "black" }]}>
                                    {isSaved ? "Saved" : "Save"}
                                </Text>
                            </TouchableOpacity>
            ...
            >
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <TouchableOpacity
                            style={[styles.submitButton, (!newReview.trim() || newRating === 0) && styles.disabledButton]}
            ...
            >
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        <View style={styles.modalHeader}>
                            <Text style={styles.modalTitle}>Reviews ({recipe?.reviews?.length || 0})</Text>
                            ...
                            {recipe?.reviews?.map((review, index) => (
                                <View key={index} style={styles.reviewItem}>
                                    ...
                                    <View style={styles.noReviews}>
                                        <Ionicons name="chatbubble-outline" size={48} color="#BDC3C7" />
                                        <Text style={styles.noReviewsText}>No reviews yet</Text>
                                        <Text style={styles.noReviewsSubtext}>Be the first to review this recipe!</Text>
                                    </View>
                                )}
                        </ScrollView>
                    </View>
